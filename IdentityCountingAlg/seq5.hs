...
solve nTuple ts = let
		nUpperbound = 2^(nTuple + 1)
	in
		Map.fromListWith (+) [ (map (\n -> evalFunk b n xs) [0..nUpperbound-1],1) | xs <- ts]
...
ct2' useOffset x (o:os) = [i : xs | i <- [(if useOffset then o else x)..5], xs <- (ct2'' (useOffset && i==o) 0 i os)] -- sæt første element som offset ved de senere!
...
firstDigitAndOffsets nTuple = scanl (\(j1,v1) (j2,v2) -> (j2, v1+v2)) (0,0) [(i ,4^(nTuple - 1)) | i<- [1..6] ]
...
computeSolution nTuple nCores = res where
	...
	res = Map.fold (+) 0 $ Map.map (\x -> x*x) $ Map.unionsWith (+) resultPar
