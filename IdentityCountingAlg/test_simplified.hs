...
solve nTuple ts = let
	...
	in
		[ xs | xs <- ts, and [evalFunk a' n xs == n | n <- [0..nUpperbound-1] ] ,  isMinimal xs] -- Vi tester til sidst om den er minimal, da dette er tungt
...
-}
main = print (concat resultPar) where
	nTuple = 10
	nCores = 4
