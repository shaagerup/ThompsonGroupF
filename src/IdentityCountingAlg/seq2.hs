...
solve nTuple ts = let
	...
	in
		[ xs | xs <- ts, and [evalFunk b n xs == n | n <- [0..nUpperbound-1] ]]
...
runTask nTuple (offset, len) = solve nTuple (take len $ map converttuple $ ct2 offset)
...
main = do
		...
		let	(x,y) = partition satisfiesRelation $ map fst res
		--mapM putStrLn $ map descSolStr res
		putStrLn $ "Cyclic:"
		mapM putStrLn $ map show x
		putStrLn $ "TOTAL: " ++ (show $ 2 * length x)
		putStrLn $ ""
		putStrLn $ "Non-cyclic:"
		mapM putStrLn $ map show y
		putStrLn $ "TOTAL: " ++ (show $ 2 * length y)
		putStrLn $ ""
